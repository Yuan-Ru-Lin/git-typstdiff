...
def main():
    ...
    # Prepare output with macros for highlighting and styling
    out = []
    out.append(f"// Generated by git-typstdiff {args.rev1}...{args.rev2} on {args.file}")
    out.append("")
    out.append("#let insert(x) = underline(stroke: 0.5pt + blue, text(blue, x))")
    out.append("#let delete(x) = strike(stroke: 0.5pt + red, text(red, x))")
    out.append("")
